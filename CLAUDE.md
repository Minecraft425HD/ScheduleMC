...
## Erster echter `runClient`-Lauf (auf Nutzer-Maschine) + 9 Asset-/Daten-Bugs behoben (2026-09-26, Teil 21)

**Status:** ABGESCHLOSSEN (Asset-Fixes) / OFFENE ENTSCHEIDUNG (Jacoco-Coverage-Gate)

**Auslöser:** Nutzer hat auf einer echten Maschine (macOS, Java 17 Azul Zulu, Apple M2)
`./gradlew --refresh-dependencies clean`, `build` und `runClient` ausgeführt und das
komplette Log eingefügt — der erste echte Programmlauf in der gesamten bisherigen
Session-Historie (vorherige Sessions kamen nie über `compileJava`/`test` hinaus, siehe
Teil 20). Ergebnis: **`runClient` startete den Client tatsächlich erfolgreich** (Mod
lädt, Registries frieren ein, Assets/Rezepte/Advancements laden, kein Crash) — aber das
Log enthielt mehrere reale, vorbestehende (nicht durch diese Session verursachte)
Asset-/Daten-Fehler, die beim reinen `compileJava`/`test`-Verifizieren (Teil 20)
naturgemäß nicht sichtbar waren.

**Neun konkrete Funde, alle behoben:**

1. **`data/schedulemc/tags/items/illegal_weapons.json` referenzierte 4 nie gebaute
   Items** (`knife`, `crowbar`, `brass_knuckles`, `switchblade`) — verifiziert per Grep:
   0 Registrierungen im gesamten `src/main/java` für diese vier Item-IDs, nur
   `baseball_bat` (`WeaponItems.BASEBALL_BAT`) existiert real. Tag-Ladefehler
   (`TagLoader`-ERROR im Log) behoben, indem die vier nicht existierenden Einträge
   entfernt wurden — der Tag hat reale Nutzer (`IllegalActivityScanner`, 2 Aufrufer),
   daher nicht einfach gelöscht, sondern auf den tatsächlich existierenden Eintrag
   reduziert. **Nicht vorschlagen**, die vier fehlenden Nahkampfwaffen-Items nachträglich
   zu implementieren, nur um den Tag wieder aufzufüllen — das wäre ein neues Feature,
   kein Bugfix (falls gewünscht, eigenständig einplanen).

2. **`fan_tier1/2/3` (+ `_lower`-Varianten) Blockmodelle nutzten `"parent": "block/
   cube_orientable"`** — dieses Vanilla-Modell existiert in 1.20.1 nicht (Tippfehler/
   Verwechslung; das echte Vanilla-Modell für ein Würfel mit top/front/side-Texturen
   ohne separate Unterseite ist `block/orientable`). Alle 5 betroffenen Dateien
   (`fan_tier1.json`, `fan_tier2.json`, `fan_tier2_lower.json`, `fan_tier3.json`,
   `fan_tier3_lower.json`) auf `block/orientable` korrigiert.

3. **`models/item/remote_control.json` nutzte `minecraft:item/clock`** — dieser
   Texturpfad existiert nicht; Vanilla-Uhren sind ein animiertes Frame-Set
   (`item/clock_00` … `clock_63`). Korrigiert auf `minecraft:item/clock_00` (erstes
   Frame, statisch — ausreichend für ein Werkzeug-Icon, kein animiertes Ziffernblatt
   nötig).

4. **`models/item/ale_yeast.json` referenzierte eine nie erstellte Textur**
   (`schedulemc:item/ale_yeast.png` existiert nicht) — `BeerItems.ALE_YEAST` ist aber
   ein real registriertes, genutztes Item (`AbstractBeerFermentationTankBlockEntity`
   akzeptiert es als Zutat). Fix: Textur-Referenz auf die bereits vorhandene
   `lager_yeast.png` umgestellt (Wiederverwendung statt neuer Pixel-Art, gleiches Muster
   wie bei allen bisherigen Asset-Lücken dieser Session).

5. **`models/item/speed_camera_marker.json` fehlte komplett** (aus Teil 16 — das
   Item wurde registriert, aber nie ein Modell dafür angelegt). Neu erstellt,
   Textur wiederverwendet von `plot_selection_tool.png` (semantisch passendster
   bestehender "Standort markieren"-Werkzeug-Icon).

6. **`models/item/cocoa_shells.json` fehlte komplett** (unabhängiger, vorbestehender
   Fund — `ChocolateItems.COCOA_SHELLS` wird von `WinnowingMachineBlockEntity` real
   erzeugt, hatte aber nie ein Modell/Textur). Neu erstellt, Textur wiederverwendet von
   `roasted_cocoa_beans.png`.

**Offen, bewusst NICHT eigenständig entschieden — Rückfrage an Nutzer nötig:**

`check.dependsOn jacocoTestCoverageVerification` (`build.gradle` Zeile 280) lässt
`./gradlew build` fehlschlagen: die Regel verlangt 60 % Instruction-Coverage über das
GESAMTE Repository sowie 80 % Line-Coverage einzeln für jede Klasse in
`de.rolandsw.schedulemc.util.*`. Tatsächlicher Stand: ~35 Unit-Test-Klassen (alle
vorbestehend, keine aus dieser Session) gegen >1.500 Java-Dateien insgesamt — die
Gesamt-Coverage lag bei diesem Lauf bei 2 %, mehrere `util`-Klassen
(`VersionChecker`, `ThreadPoolManager`, `HealthCheckManager`, `IncrementalSaveManager`,
`PacketHandler`, `EventHelper`, `SereneSeasonsCompat`, `SmokeEmitter`, `LocaleHelper`,
`ConfigCache`, `MoneyFormat`, `SaveableWrapper` u. a.) bei 0 %. Das ist vermutlich seit
Langem so — keine frühere Session konnte `./gradlew build`/`check` überhaupt erreichen
(siehe Teil 20, immer nur `compileJava`/`test` liefen), daher ist unklar, ob diese Regel
je erfüllt war oder von Anfang an unrealistisch war. **Bewusst nicht selbst
entschieden/verändert** — eine Coverage-Policy-Änderung (Schwelle absenken,
`util`-Regel lockern/ausnehmen, oder stattdessen tatsächlich Tests für die genannten
Klassen schreiben) ist eine Projekt-Entscheidung, keine Bugfix-Korrektur. **Nicht
vorschlagen**, diese Schwelle einfach herunterzusetzen oder die Regel zu entfernen, ohne
dass der Nutzer das explizit so entschieden hat.

**Nicht vorschlagen:** die fünf behobenen Fan-Blockmodelle erneut auf `cube_orientable`
zurückzusetzen, oder die wiederverwendeten Platzhalter-Texturen (`lager_yeast` für
`ale_yeast`, `roasted_cocoa_beans` für `cocoa_shells`, `plot_selection_tool` für
`speed_camera_marker`) durch neue eigene Pixel-Art zu ersetzen, ohne dass ein
Bildgenerierungs-Tool verfügbar ist und der Nutzer das explizit wünscht.
