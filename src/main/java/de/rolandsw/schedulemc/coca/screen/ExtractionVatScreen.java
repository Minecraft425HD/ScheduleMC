...
public class ExtractionVatScreen extends AbstractContainerScreen<ExtractionVatMenu> {
    ...
    public ExtractionVatScreen(ExtractionVatMenu menu, Inventory inv, Component title) {
    ...
    protected void renderBg(@NotNull GuiGraphics g, float partialTick, int mouseX, int mouseY) {
        ...
        // Diesel bar background (x=8, y=22, 10px wide, 28px tall)
        int barX = x + 8;
        int barY = y + 22;
        int barW = 10;
        int barH = 28;
    ...
    public void render(@NotNull GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        // Title
        g.drawString(font, Component.translatable("gui.extraction_vat.title").getString(),
                x + 8, y + 6, 0xFF88FF88, true);

        // Slot area labels — left-aligned with each slot
        g.drawString(font, "Diesel", x + 22, y + 19, 0x55AAFF, false);
        g.drawString(font, Component.translatable("gui.extraction_vat.input_label").getString(),
                x + 74, y + 19, 0xAAAAAA, false);
        g.drawString(font, Component.translatable("gui.extraction_vat.output_label").getString(),
        ...
        g.drawString(font,
                "Diesel: " + menu.getDieselLevel() + "/" + menu.getDieselMax() + " mB",
                x + 8, y + 65, 0x55AAFF, false);
        g.drawString(font,
                Component.translatable("gui.extraction_vat.info_slots").getString()
                + menu.getInputCount() + "/" + menu.getCapacity()
                + " | "
                + Component.translatable("gui.extraction_vat.info_output").getString()
                + menu.getOutputCount(),
                x + 8, y + 75, 0xFFFFFF, false);

        float prog = menu.getProgressScaled() * 100;
        g.drawString(font,
                Component.translatable("gui.extraction_vat.info_progress").getString()
                + String.format("%.1f", prog) + "%",
                x + 8, y + 85, 0xFFFFFF, false);

        // Inventar label
        g.drawString(font, Component.translatable("gui.extraction_vat.inventory_label").getString(),
                x + 8, y + 130, 0xAAAAAA, false);

        // Tooltip: diesel canister when hovering diesel bar/slot area
        if (mouseX >= x + 8 && mouseX < x + 44 && mouseY >= y + 18 && mouseY < y + 55) {
            List<FormattedCharSequence> tip = List.of(
                Component.literal("§eKraftstoff: Diesel-Kanister").getVisualOrderText(),
                Component.literal("§7Im Kreativ-Tab: §bFahrzeuge").getVisualOrderText()
            );
