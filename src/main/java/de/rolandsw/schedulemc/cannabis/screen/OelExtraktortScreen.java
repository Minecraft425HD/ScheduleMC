...
public class OelExtraktortScreen extends AbstractContainerScreen<OelExtraktortMenu> {
    ...
    public OelExtraktortScreen(OelExtraktortMenu menu, Inventory playerInventory, Component title) {
    ...
    private void renderActionButton(GuiGraphics graphics, int x, int y, int mouseX, int mouseY) {
        ...
        if (menu.hasOutput()) {
            text  = Component.translatable("gui.oel_extraktor.oil_ready").getString();
            color = 0xFF335533;
        } else if (menu.isExtracting()) {
            text  = Component.translatable("gui.oel_extraktor.extracting").getString();
            color = 0xFF333355;
        } else if (menu.canStart()) {
            color = hovered ? 0xFF55DD55 : 0xFF44AA44;
            text  = Component.translatable("gui.oel_extraktor.button_start").getString();
        } else {
            text  = Component.translatable("gui.oel_extraktor.button_start").getString();
            color = 0xFF2A3A2A;
        }
    ...
    public void render(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.drawString(this.font,
                Component.translatable("gui.oel_extraktor.title").getString(),
                x + 10, y + 8, 0xFF88FF88, true);

        if (menu.hasOutput()) {
            String ready = Component.translatable("gui.oel_extraktor.oil_ready").getString();
