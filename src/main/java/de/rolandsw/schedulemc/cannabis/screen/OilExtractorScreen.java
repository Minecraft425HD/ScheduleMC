...
public class OilExtractorScreen extends AbstractContainerScreen<OilExtractorMenu> {
    ...
    private static final int BAR_Y      = 85;
    ...
    private static final int STATUS_Y = 143;

    public OilExtractorScreen(OilExtractorMenu menu, Inventory playerInventory, Component title) {
    ...
    private void renderInfoRows(GuiGraphics graphics, int x, int y) {
        ...
        String row1 = Component.translatable("gui.oel_extractor.info_material").getString()
        ...
        String row2 = Component.translatable("gui.oel_extractor.info_solvent").getString()
                    + menu.getSolventCount();
        graphics.drawString(this.font, row2, x + 10, y + ROW2_Y, 0xFFFFFF, false);

        String row3 = Component.translatable("gui.oel_extractor.info_oil").getString()
    ...
    private void renderProgressBar(GuiGraphics graphics, int x, int y) {
        ...
        int filled = (int)(menu.getExtractionProgressF() * BAR_WIDTH);
        if (filled > 0) {
    ...
    public void render(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.drawString(this.font,
                Component.translatable("gui.oel_extractor.title").getString(),
                x + 10, y + 8, 0xFF88FF88, true);

        if (menu.hasOutput()) {
            String ready = Component.translatable("gui.oel_extractor.oil_ready").getString();
        ...
        if (!menu.isExtracting() && !menu.hasOutput()) {
            if (menu.getSolventCount() < 1) {
                graphics.drawString(this.font, Component.translatable("gui.oel_extractor.hint_solvent_needed").getString(), x + 10, y + STATUS_Y, 0xAAAAAA, false);
            } else if (menu.getMaterialWeight() < OilExtractorBlockEntity.MIN_MATERIAL_WEIGHT) {
                String warn = String.format(Component.translatable("gui.oel_extractor.hint_min_material").getString(),
