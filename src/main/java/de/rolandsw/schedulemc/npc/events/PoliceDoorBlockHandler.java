...
public class PoliceDoorBlockHandler {
    ...
    public void onDoorInteract(PlayerInteractEvent.RightClickBlock event) {
        ...
        List<CustomNPCEntity> nearbyPolice = player.level().getEntitiesOfClass(
            ...
            AABB.ofSize(player.position(), detectionRadius, detectionRadius, detectionRadius),
        ...
        if (!nearbyPolice.isEmpty()) {
            ...
            player.sendSystemMessage(Component.literal("§c✗ Du kannst keine Türen öffnen, während die Polizei dich verfolgt!"));
            player.sendSystemMessage(Component.literal("§7Verstecke dich vor der Polizei, um Türen zu benutzen."));
        }
    }
}
