 ...
 * Preserves quality
 */
public class HoneyExtractorBlockEntity extends AbstractItemHandlerBlockEntity implements IUtilityConsumer, MenuProvider {
    private static final Logger LOGGER = LoggerFactory.getLogger(HoneyExtractorBlockEntity.class);
            ...
            if (tag != null) {
                if (tag.contains("HoneyType")) {
                    try { honeyType = HoneyType.valueOf(tag.getString("HoneyType")); }
                        ...
                        LOGGER.warn("Invalid HoneyType '{}' in HoneyExtractorBlockEntity at {}", tag.getString("HoneyType"), getBlockPos(), exception);
                    }
                }
