...
public class ReductionKettleBlockEntity extends BlockEntity implements IUtilityConsumer {
    ...
    private MethQuality inputQuality = MethQuality.POOR;

    private UUID activePlayer = null; // Spieler der gerade die GUI bedient

    public ReductionKettleBlockEntity(BlockPos pos, BlockState state) {
        super(MethBlockEntities.REDUCTION_KETTLE.get(), pos, state);
    }
    ...
    private void completeProcess() {
        ...
        if (dangerPercent > 0.3) {
            ...
            if (level != null && level.random.nextFloat() < 0.3) { // 30% Chance
                finalQuality = MethQuality.LEGENDARY;
            } else {
        ...
        outputItem = RawMethItem.create(finalQuality, 1);
        isProcessing = false;
