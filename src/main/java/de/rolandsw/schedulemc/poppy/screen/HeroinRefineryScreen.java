...
public class HeroinRefineryScreen extends AbstractContainerScreen<HeroinRefineryMenu> {
    ...
    public HeroinRefineryScreen(HeroinRefineryMenu menu, Inventory inv, Component title) {
    ...
    public void render(@NotNull GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        g.drawString(font, Component.translatable("gui.heroin_refinery.title").getString(),
                x + 8, y + 6, 0xFF88FF88, true);

        g.drawString(font, Component.translatable("gui.heroin_refinery.fuel_label").getString(),
                x + 22, y + 19, 0xFFAA44, false);
        g.drawString(font, Component.translatable("gui.heroin_refinery.input_label").getString(),
                x + 74, y + 19, 0xAAAAAA, false);
        g.drawString(font, Component.translatable("gui.heroin_refinery.output_label").getString(),
        ...
        g.drawString(font,
                Component.translatable("gui.heroin_refinery.fuel_label").getString()
                + ": " + menu.getFuelLevel() + "/" + menu.getFuelMax(),
                x + 8, y + 65, 0xFFAA44, false);
        g.drawString(font,
                Component.translatable("gui.heroin_refinery.info_slots").getString()
                + menu.getInputCount() + "/" + menu.getCapacity()
                + " | "
                + Component.translatable("gui.heroin_refinery.info_output").getString()
                + menu.getOutputCount(),
                x + 8, y + 75, 0xFFFFFF, false);

        float prog = menu.getProgressScaled() * 100;
        g.drawString(font,
                Component.translatable("gui.heroin_refinery.info_progress").getString()
                + String.format("%.1f", prog) + "%",
                x + 8, y + 85, 0xFFFFFF, false);

        g.drawString(font, Component.translatable("gui.heroin_refinery.inventory_label").getString(),
        ...
        if (mouseX >= x + 8 && mouseX < x + 44 && mouseY >= y + 30 && mouseY < y + 58) {
            List<FormattedCharSequence> tip = List.of(
                ...
                Component.literal("§7Coal Block (×800)").getVisualOrderText(),
